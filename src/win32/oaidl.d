...
interface ITypeInfo : IUnknown {
	HRESULT GetTypeAttr(out LPTYPEATTR);
	HRESULT GetTypeComp(out LPTYPECOMP);
	HRESULT GetFuncDesc(UINT, out LPFUNCDESC);
	HRESULT GetVarDesc(UINT, out LPVARDESC);
	HRESULT GetNames(MEMBERID, BSTR*, UINT, out UINT);
	HRESULT GetRefTypeOfImplType(UINT, out HREFTYPE);
	HRESULT GetImplTypeFlags(UINT, out INT);
	HRESULT GetIDsOfNames(LPOLESTR*, UINT, out MEMBERID);
	...
	HRESULT GetRefTypeInfo(HREFTYPE, out LPTYPEINFO);
	...
	HRESULT GetContainingTypeLib(out LPTYPELIB, out UINT);
...
interface ITypeInfo2 : ITypeInfo {
    ...
    HRESULT GetCustData(const ref GUID guid, out VARIANT pVarVal);
    HRESULT GetFuncCustData(uint index, const ref GUID guid, out VARIANT pVarVal);
    HRESULT GetParamCustData(uint indexFunc, uint indexParam, const ref GUID guid, out VARIANT pVarVal);
    HRESULT GetVarCustData(uint index, const ref GUID guid, out VARIANT pVarVal);
    HRESULT GetImplTypeCustData(uint index, const ref GUID guid, out VARIANT pVarVal);
    HRESULT GetDocumentation2(int memid, uint lcid, wchar** pBstrHelpString, uint* pdwHelpContext, wchar** pBstrHelpStringDll);
    ...
    HRESULT GetAllTypeImplCustData(uint index, out CUSTDATA pCustData);
}
alias ITypeInfo2 LPTYPEINFO2;

interface ITypeLib : IUnknown {
	UINT GetTypeInfoCount();
	HRESULT GetTypeInfo(UINT, out LPTYPEINFO);
	HRESULT GetTypeInfoType(UINT, out TYPEKIND);
	HRESULT GetTypeInfoOfGuid(REFGUID, out LPTYPEINFO);
	HRESULT GetLibAttr(out TLIBATTR*);
	HRESULT GetTypeComp(out ITypeComp);
	HRESULT GetDocumentation(INT, BSTR*, BSTR*, DWORD*, BSTR*);
	HRESULT IsName(LPOLESTR, ULONG, out BOOL);
	HRESULT FindName(LPOLESTR, ULONG, LPTYPEINFO*, MEMBERID*, ref USHORT);
