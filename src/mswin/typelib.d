                ...
                if (typeInfo.GetContainingTypeLib(typeLib, index) == S_OK) {
                    ...
                    if (typeLib.GetLibAttr(libAttr) == S_OK) {
                        ...
                        if (!(libAttr.guid in map)) {
                            ...
                            scope typeLibKey = RegistryKey.classesRoot.openSubKey("TypeLib\\" ~ libAttr.guid.toString());
                            if (typeLibKey !is null && typeLibKey.subKeyCount > 0) {
    ...
    final Module[] getModules() {
        if (modules_ == null) {
            ...
            for (auto i = 0; i < typeLib_.GetTypeInfoCount(); i++) {
                checkResult(typeLib_.GetTypeInfoType(i, typeKind));
                if (typeKind == TYPEKIND.TKIND_MODULE) {
                    checkResult(typeLib_.GetTypeInfo(i, typeInfo));
    ...
    final Type[] getTypes() {
        if (types_ == null) {
            ...
            for (auto i = 0; i < typeLib_.GetTypeInfoCount(); i++) {
                checkResult(typeLib_.GetTypeInfo(i, typeInfo));
                checkResult(typeLib_.GetTypeInfoType(i, typeKind));
                checkResult(typeInfo.GetTypeAttr(typeAttr));
                ...
                switch (typeKind) {
                    ...
                    case TYPEKIND.TKIND_DISPATCH:
                        ...
                        if (hr != TYPE_E_ELEMENTNOTFOUND && refType != 0) {
                            ITypeInfo refTypeInfo;
                            checkResult(typeInfo.GetRefTypeInfo(refType, refTypeInfo));
...
class Module {
    ...
    */
    final Member[] getMembers() {
        if (members_ == null) {
            TYPEATTR* typeAttr;
            checkResult(typeInfo_.GetTypeAttr(typeAttr));
            ...
            for (auto i = 0; i < typeAttr.cFuncs; i++) {
                checkResult(typeInfo_.GetFuncDesc(i, funcDesc));
                    ...
                    if (dispTypeInfo_ !is null) {
                        ...
                        if (hasBase && (attributes & TypeAttributes.InterfaceIsDispatch)) {
                            if (typeAttr.typekind == TYPEKIND.TKIND_INTERFACE || typeAttr.typekind == TYPEKIND.TKIND_DISPATCH) {
                                checkResult(baseTypeInfo.GetRefTypeOfImplType(0, refType));

                                ITypeInfo realTypeInfo;
                                checkResult(baseTypeInfo.GetRefTypeInfo(refType, realTypeInfo));
                            ...
                            checkResult(typeInfo.GetRefTypeOfImplType(0, refType));
                            ITypeInfo realTypeInfo;
                            checkResult(baseTypeInfo.GetRefTypeInfo(refType, realTypeInfo));
    ...
    override Type[] getInterfaces() {
        if (interfaces_ == null && isCoClass()) {
            ...
            for (uint i = 0; i < count; i++) {
                uint refType;
                checkResult(typeInfo_.GetRefTypeOfImplType(i, refType));

                ITypeInfo implTypeInfo;
                checkResult(typeInfo_.GetRefTypeInfo(refType, implTypeInfo));
                scope(exit) implTypeInfo.Release();

                int flags;
                checkResult(typeInfo_.GetImplTypeFlags(i, flags));
                TypeAttributes attrs = TypeAttributes.Interface;
...
class Parameter {
    ...
    */
    ...
    package static Parameter[] getParameters(MethodImpl method, out Parameter returnParameter, bool getReturnParameter) {
        ...
        checkResult(method.typeInfo_.GetTypeAttr(typeAttr));
        scope(exit) method.typeInfo_.ReleaseTypeAttr(typeAttr);

        for (uint i = 0; i < typeAttr.cFuncs; i++) {
            checkResult(method.typeInfo_.GetFuncDesc(i, funcDesc));
            ...
            if (funcDesc.memid == method.id && (funcDesc.invkind & method.attributes)) {
                ...
                checkResult(method.typeInfo_.GetNames(funcDesc.memid, bstrNames, funcDesc.cParams + 1, count));
